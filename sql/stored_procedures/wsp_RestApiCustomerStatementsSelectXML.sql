...
-- Description:	Stored procedure for SELECTing customer statements for the WinMan REST API in XML format.
-- =============================================
...
(
    ...
    WHERE
        p.[type] = 'P'
		AND p.[name] = 'wsp_RestApiCustomerStatementsSelectXML'
		AND s.[name] = 'dbo'
)
	BEGIN
		EXECUTE('CREATE PROCEDURE dbo.wsp_RestApiCustomerStatementsSelectXML AS PRINT ''dbo.wsp_RestApiCustomerStatementsSelectXML''');
	END;
GO

-- 27Mar18 LAE Added total row count and changed ordering to DESC.

ALTER PROCEDURE [dbo].[wsp_RestApiCustomerStatementsSelectXML]
	@pageNumber int = 1,
	@pageSize int = 20,
...
BEGIN
	...
	IF dbo.wfn_BespokeSPExists('bsp_RestApiCustomerStatementsSelectXML') = 1 
		BEGIN
			EXEC dbo.bsp_RestApiCustomerStatementsSelectXML
	...
	SELECT @results = 
		...
		ORDER BY
			rowNumber
		-- 27Mar18 LAE
		--FOR XML PATH('CustomerStatement'), TYPE));
		FOR XML PATH('CustomerStatement'), TYPE)), @total = (SELECT COUNT(*) FROM Statements);		

	IF @results IS NOT NULL AND @results <> ''
		BEGIN
			SELECT @results = '<CustomerStatements>' + @results + '</CustomerStatements>';
		END;
	ELSE
		BEGIN
			SELECT @results = '<CustomerStatements/>';
		END;
