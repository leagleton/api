...
-- Description:	Function to SELECT product stock levels for the WinMan REST API in JSON format.
-- =============================================
...
(
	...
	FROM
		sys.objects
	WHERE
		[object_id] = OBJECT_ID(N'[dbo].[wfn_RestApiGetStockLevelsJSON]')
		AND [type] IN (N'FN', N'IF', N'TF', N'FS', N'FT')
)
	BEGIN
		EXECUTE('CREATE FUNCTION dbo.wfn_RestApiGetStockLevelsJSON() RETURNS nvarchar(100) AS BEGIN RETURN ''dbo.wfn_RestApiGetStockLevelsJSON''; END;');
	END;
GO

ALTER FUNCTION [dbo].[wfn_RestApiGetStockLevelsJSON] 
(
	@Product bigint,
	@Site bigint
)
...
BEGIN
			...
			FROM
				(SELECT
					...
					WHERE SI.Product = @Product
						...
						AND NOT S.Customer IN (SELECT Customer
				FROM Sites)
	...
	DECLARE @stockLevels nvarchar(150);

	SET @stockLevels = '"QuantityInStock":' + CAST(ISNULL(@QuantityOnHand,0) AS nvarchar(18)) + ','
						+ '"QuantityHardAllocated":' + CAST(ISNULL(@QuantityHardAllocated,0) AS nvarchar(18)) + ','
						+ '"QuantitySoftAllocated":' + CAST(ISNULL(@QuantitySoftAllocated,0) AS nvarchar(18));
