    ...
    return (
      <pre className="errors-wrapper">
        <hgroup className="error">
          <h5 className="errors__title">Errors</h5>
...
const ThrownErrorItem = ({ error, jumpToLine }) => {
  ...
  return (
    <div className="error-wrapper">
      {!error ? null :
        <div>
          <h6>{(error.get("source") && error.get("level")) ?
            toTitleCase(error.get("source")) + " " + error.get("level") : ""}
            {error.get("path") ? <small> at {error.get("path")}</small> : null}</h6>
...
const SpecErrorItem = ({ error, jumpToLine }) => {
  ...
  return (
    <div className="error-wrapper">
      {!error ? null :
        <div>
          <h6>{toTitleCase(error.get("source")) + " " + error.get("level")}&nbsp;{locationMessage}</h6>
