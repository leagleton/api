...
export const sampleFromSchema = (schema, config = {}) => {
  let { type, example, properties, additionalProperties, items, xml } = objectify(schema)
  ...
  if (type === "object") {
    ...
    for (var name in props) {
      if (!props[name].readOnly || includeReadOnly) {
        if (props[name].hasOwnProperty('xml')) {
          props[name].xml.wrapped = false
        }
  ...
  if (type === "array") {
    var wrapped = (xml.hasOwnProperty('wrapped')) ? xml.wrapped : '';
    if (wrapped === true) {
      let obj = {}
      obj[xml.name] = [sampleFromSchema(items, { includeReadOnly: includeReadOnly })]
      return obj
    } else {
