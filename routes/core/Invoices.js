...
/**
 * Fetch (GET) a customer's invoice from WinMan.
 */
router.get('/', passport.authenticate('bearer', { session: false }), function (req, res, next) {
    const inputParams = [];
    const scopes = req.authInfo.scope.split(',');

    if (scopes.indexOf('getInvoices') === -1) {
        res.status(403);
        res.send(utils.reasons.inadequateAccess);
        return;
    } else {
        inputParams.push("@scope = 'getInvoices'");
    }
    ...
    if (typeof res.locals.salesinvoiceid !== 'undefined') {
        inputParams.push("@salesInvoiceId = '" + res.locals.salesinvoiceid + "'");
    } else {
        return utils.reject(res, req, utils.reasons.requiredParam
            + ' salesinvoiceid.');
    }
    ...
    const params = {
        sp: 'wsp_RestApiCustomerInvoicesSelect',
        args: inputParams
    };
