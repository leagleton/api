...
CREATE FUNCTION [dbo].[wfn_RestApiGetStockLevelsJSON] 
(
	@Product bigint,
	@Site bigint
)
...
BEGIN
	...
	DECLARE
		@QuantityOnHand			decimal(17,5),
		@QuantityHardAllocated	decimal(17,5),
		@QuantitySoftAllocated	decimal(17,5)
	...
	DECLARE @stockLevels NVARCHAR(200);

	SET @stockLevels = '"QuantityInStock":' + CAST(ISNULL(@QuantityOnHand,0) AS NVARCHAR(10)) + ','
						+ '"QuantityHardAllocated":' + CAST(ISNULL(@QuantityHardAllocated,0) AS NVARCHAR(10)) + ','
						+ '"QuantitySoftAllocated":' + CAST(ISNULL(@QuantitySoftAllocated,0) AS NVARCHAR(10));
